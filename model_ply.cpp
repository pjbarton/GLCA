...
void seekLine(FILE *file, char *buffer, char const seekString[])
{
...
int Model_PLY::Load(char* filename) 
{
	...
	if (file)
	{
		...
		// read vertices
		for (int iterator = 0, j = 0; iterator < this->NumVertices; iterator++, j += 3)
		{
			fgets(buffer, 300, file);
			sscanf(buffer,"%f %f %f", &Vertices[j], &Vertices[j+1], &Vertices[j+2]);
		}
